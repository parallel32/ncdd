...
$utilities->get('/resendseminarconfirmationletter', function () use ($app) {
    ...
    $reg_arr[] = '5683edd954fe0b5e79742c16';
...
$utilities->get('/memberswithoutautorenew', function () use ($app) {
    ...
    foreach ($members as $member){
        ...
        if(!empty($member['renewal']) && is_array($member['renewal']) && array_key_exists('applicationId', $member['renewal'])){
...
$utilities->get('/memberswithautorenew', function () use ($app) {
    ...
    foreach ($members as $member){
        ...
        $res_arr['expMonth'] = (array_key_exists('expMonth', $member['payment'])) ? $member['payment']['expMonth'] : '';
        $res_arr['expYear'] = (array_key_exists('expYear', $member['payment'])) ? $member['payment']['expYear'] : '';
...
$utilities->get('/scrubmembers', function () use ($app) {
    ...
    // "Ahmad Moosavi Shabani ","Shabani Law Group, LLC","3156 Pelham Parkway ","Suite 2B ","","","Pelham","AL","35124"," Office  (205) 624-4043   Office  (205) 624-4044   Fax  (205) 624-4045  "
        ...
        if (($handle = fopen("/var/www/upload/avvo_attorneys.csv", "r")) !== FALSE) {
            while (($data = fgetcsv($handle)) !== FALSE) {
                ...
                for ($c=0; $c < $num; $c++) {
                    ...
                    $datar = trim($data[$c]);
                    if($c < 10){
                        $output[$row][$fields[$c]] = $datar;
                    }
                    
                }
                echo "<pre>-----output row: ";print_r($output[$row]);echo "</pre>";    
                if(!empty($output[$row]['name']) && strlen($output[$row]['name']) > 0){
                    ...
                    if(!empty($results)){
                        ...
                        if($row > 2){
                            if(!empty($output[$row]['address1'])){
                                fputcsv($fp, $output[$row]);
                                echo '<pre>added ';print_r($output[$row]);echo '</pre>';
                                $wodups++;                            
                            }
                        }
