...
$app['pushToCDN'] = $app->protect(function ($app,$imageId,$belongsTo,$belongsToType=null,$groups=array()) {
    ...
    $s3 = new \AmazonS3();
    $s3->path_style = true;
    $SAW_cdn = str_replace('http://','',SAW_CDN);
    $SAW_cdn = str_replace('https://','',$SAW_cdn);
    $bucket = $SAW_cdn;
...
$app->post('/images/upload', function (Request $request) use ($app) {
	try{
...
$app->post('/images/crop', function (Request $request) use ($app) {
	try{
		$context = $app['request']->get('context');
		$belongsTo = $app['request']->get('belongsTo');
...
//error_log('cropped_image_uri:'.$cropped_image_uri);
				...
				if ($img_create['status']) {
					...
					if($response->isOK()):
						// create urls for the response
						$urls[$key] = SAW_CDN.'/'.$filename.'?v='.time().rand();
					else:
						// throw exception... to be caught by c.images.php to be sent down as a failure response to upload_overlay_s3.php
						error_log('UploadWrapperS3::update::response::'.print_r($response,true));
					endif;
 ...
 */
$app->get('/images/{type}/{size}/{id}', function ($type, $size, $id, Request $request) use ($app) {
	try{
		if(strpos($type, 'qr') !== false){
			$qr = true;
			$split = explode('qr-',$type);
			$type = $split[1];
			
			// type represents all of the qr image types
			switch ($type) {
				case 'offer':
					$doc = Model\Offer::getQR($app,array('_id'=>new \MongoId($id)),'offer');
					$code = $doc['QR']['code'];
                    $accentColor = array('R'=>151, 'G'=>104, 'B'=>121); //purple
					break;
				case 'profile':
					$doc = Model\Profile::getQR($app,array('_id'=>new \MongoId($id)),'profile');
					$code = $doc['QR']['code'];
                    $accentColor = array('R'=>151, 'G'=>104, 'B'=>121); //purple
					break;
                case 'setup':
				case 'merchant':
					$doc = Model\Merchant::getQR($app,array('_id'=>new \MongoId($id)),'merchant');
					$code = $doc['QR']['code'];
                    $accentColor = array('R'=>192, 'G'=>0, 'B'=>0); //red
					break;
				case 'grape':
					$doc = Model\Grape::getQR($app,array('_id'=>new \MongoId($id)),'grape');
					$code = $doc['QR']['code'];
                    $accentColor = array('R'=>151, 'G'=>104, 'B'=>121); //purple
					break;
			}
			
			return $app['QRResponse']($code, $size, $accentColor);
		}else{
            // get other non CDN images
            
			return new Response('',404,array('Content-Type' => 'image/jpeg'));
		}
	} catch (Saw\Exceptions\SawException $e) {
		$http_status_code = $e->getHttpStatusCode();
		$abort_response = $e->getAbortResponse();  
