...
$app['renewal_card_expiration_date'] = $app->protect(function () use ($app) {
	...
	if($user['accessLevel'] != ADMIN){
		$member = new Model\Member(array('_id'=>$user['_id']),$app);
		$member = $member->findById();
		if(is_array($member) && array_key_exists('payment',$member) && array_key_exists('number',$member['payment']) && !empty($member['payment']['number']) && strlen($member['payment']['number']) > 4 && array_key_exists('expMonth',$member['payment']) && $member['payment']['expMonth'] && strtotime('now') > strtotime($member['payment']['expMonth'].'/01/'.$member['payment']['expYear'])){
			return true;
		}else{
			return false;
		}
